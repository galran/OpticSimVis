...
mutable struct App <: OpticSimVis.AbstractUIApp
...
function prop(app::App, p::Symbol, default_val = nothing)
    return get(props(app), p, default_val)
end
function prop!(app::App, p::Symbol, val::Any)
    props(app)[p] = val
end
...
function fix_controls!(app::App, controls::Vector{UIControls.AbstractUIControl})
    for control in controls
        ...
        if (control.type == "button" && control.variable === nothing)
            ...
            auto_var = OpticSimVis.UIVariables.Variable(
...
function OpticSimVis.updateVariable!(app::App, var::UIVariables.AbstractUIVariable)
    if (app !== var._app)
    ...
    Blink.@js_ OpticSimVis.win(app) begin
